<template>
  <div class="grid grid-cols-1 gap-4">
    ...
    <px-table-pabellones @delete-from-lista="borrarPorLista($event)" :pabellones="pabellones" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    guardarPabellon() {
      ...
      api.postPabellon(pabellon).then(result => {
    ...
    cargarBusqueda() {
      ...
      this.telefono = this.pabellones[this.indexB].Telefono;
      ...
      if (mensaje) {
        ...
        api
          ...
          .then(result => {
            ...
            this.pabellones[this.indexB].Telefono = result.data.Telefono;
            console.log(this.pabellones[this.indexB]);
            this.indexB = -1;
            this.busqueda = "";
          });
    ...
    borrarPabellon() {
      ...
      if (mensaje) {
        api.deletePabellon(this.pabellones[this.indexB].id).then(result => {
    ...
    borrarPorLista(id) {
      ...
      if (mensaje) {
        ...
        api.deletePabellon(id).then(result => {
          if (result.status === 200) {
